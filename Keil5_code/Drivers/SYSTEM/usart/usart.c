 ...
 *  bit13~0，    接收到的有效字节数目
*/
uint8_t Useful_Light[11]={0};
uint16_t g_usart_rx_sta = 0;
uint8_t Light_Buffer[4];
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    if(huart->Instance == USART_UX)                             /* 如果是串口1，进行串口1 MSP初始化 */
    {
        ...
        HAL_NVIC_SetPriority(USART_UX_IRQn, 3, 2);              /* 抢占优先级3，子优先级3 */
#endif
    }
        else if(huart->Instance == USART3)                             /* 如果是串口1，进行串口1 MSP初始化 */
    {
        __HAL_RCC_USART3_CLK_ENABLE();                                  /* USART1 时钟使能 */
        ...
        gpio_init_struct.Pull = GPIO_PULLUP;                    /* 上拉 */
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;          /* 高速 */
        gpio_init_struct.Alternate = GPIO_AF7_USART3;          /* 复用为USART1 */
        HAL_GPIO_Init(GPIOC, &gpio_init_struct);   /* 初始化发送引脚 */

        gpio_init_struct.Pin = GPIO_PIN_11;               /* RX引脚 */
        gpio_init_struct.Alternate = GPIO_AF7_USART3;          /* 复用为USART1 */
        HAL_GPIO_Init(GPIOC, &gpio_init_struct);   /* 初始化接收引脚 */
...
//    }
    ...
    {
          if(g_rx_buffer[0]==0xff&&USART3_Count==0)
          {
              Light_Buffer[USART3_Count] = g_rx_buffer[0];
              USART3_Count+=1;
              
          }
          else if(USART3_Count==1&&g_rx_buffer[0]<11)
          {
              Light_Buffer[USART3_Count] = g_rx_buffer[0];
              USART3_Count+=1;
          }
          else if(USART3_Count==2)
          {
              Light_Buffer[USART3_Count] = g_rx_buffer[0];
              USART3_Count+=1;
          }
          else if(USART3_Count==3&&g_rx_buffer[0]==0xfe)
          {
              Light_Buffer[USART3_Count] = g_rx_buffer[0];
              Useful_Light[Light_Buffer[1]] = Light_Buffer[2];
              USART3_Count=0;
          }
          else
          {
...
{
    g_uart3_handle.Instance = USART3;                         /* USART1 */
...
//        if(USART3_Finsh_Time<9)
//        {
//            if(g_rx_buffer[0]==0xFF&&USART3_Count==0)
//            {
//                USART3_Flag=1;
//                Light_Buffer[USART3_Finsh_Time][USART3_Count] = g_rx_buffer[0];
//                USART3_Count+=1;
//            }
//            else if(USART3_Count<4&&g_rx_buffer[0]!=0xFE)
//            {
//                Light_Buffer[USART3_Finsh_Time][USART3_Count] = g_rx_buffer[0];
//                USART3_Count+=1;
//            }
...
//            }
//        }
//        else if(USART3_Finsh_Time==9)
//        {

//            for(uint8_t i=0;i<9;i++)
//            {
//                if(Light_Buffer[i][1] == i)
//                {
//                    USART3_Finsh_Time = 9;
//                }
//                else
